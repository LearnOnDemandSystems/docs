...
- After defining a result of false, we define our "ItemId". This is however you would like to identify the item. You may already have a structure for how you define your exam items, or it could be some simple within the context of just that lab "Storage Account".

- Lastly, define any custom variables you will need within your script. While the Lab on Demand replacement tokens will work properly and replace as text anywhere in your script, it is a best practice to actually turn them into a variable for the scripting format you are designing so that bulk of the script is natural to the scripting language.
...
- Within the "Try" section of your try/catch, contain each command within a variable so that it can be referenced later.
    - This is especially helpful when using object based languages such as PowerShell.

## Function Section {function}
After establishing the commands needed to determine the items desired to score, we must establish what we desire the value of those items to be and the user's configuration of those items are.
...
- Item definitions are stored within a function so that we can define it now but run it later in the script.
- We will create an object and store it in a variable for both the user's configuration as well as the correct configuration.
- Notice that the values for the user's configuration all reference the item(s) we identified that we will be scoring and stored in a variable. This section then extracts sub-properties that we will specifically be scoring.
- The values for the correct item are what we expect those user values to be.

> [!NOTE] There are other formats that could be used for scoring such as chains of if statements, this method presented here permits simple comparison values and allows us to output both user and expected values in an easy to read format later.
...
- Here we identify the opening of an "if" statement where we check that our scored item exists. 
    - If it does exist, the script will continue with scoring. 
    - If it does not exist, the script will mark the item incorrect.
...
This section is where all the scoring occurs. A best practice is to keep this as consistent across your items as possible so that most of the script can be simply copy/pasted from item to item, with the only modifications needed being the few lines outlined earlier.
