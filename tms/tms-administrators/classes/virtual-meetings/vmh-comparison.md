...
Once you have a Zoom contract, you will need to set up a Server-to-Server OAuth application through Zoom, and send information to our help desk to implement an [integration.](/tms/tms-administrators/classes/virtual-meetings/integrated-virtual-meetings.md) Once in place, your Zoom VMH is the default selection for virtual delivery. Scheduling a class automatically creates a Zoom meeting when a licensed Zoom user is added as an instructor. When a Zoom meeting scheduled through the TMS is opened, it will show in the UTC time zone.

Once in place, your Zoom VMH is the default selection for virtual delivery. Scheduling a class automatically creates a Zoom meeting when a licensed Zoom user is added as an instructor. When a Zoom meeting scheduled through the TMS is opened, it will show in the UTC time zone.

- The account used to set up the Server-to-Server OAuth application is where the meetings will be located. The primary instructor in a class must be licensed from the same account that is set up on the Server-to-Server OAuth application in order to be verified through the API. Your instructor's email address in their Skillable TMS profile must match the email address in your Zoom account for them to be verified. Your contract with Zoom provides a pool of licenses that can be assigned to different users; you may switch out instructors on the licenses in your contract.
...
- Students and Instructors may not be identified with their Skillable TMS username when they enter a Zoom meeting. This depends on how Zoom identifies them; different factors include if they have attended a Zoom meeting before, the username on their device, or using multiple instructors in a class.
...
- The Primary Instructor is assigned as the meeting Host by default (this is determined by the Instructor during the initial meeting setup/creation process). 
    - There can only be one active Host per meeting session; one Host is needed at all times for once a session is in-progress. 
    - If an additional instructor joins a meeting that is in-session, then they are automatically assigned as a meeting Co-Host upon entering. 
    - If a Co-Host joins a meeting before the primary Host arrives, then the Co-Host is assigned the Host privileges temporarily, until the primary Host arrives. After the primary host arrives, the Host privileges are then automatically transferred to the Primary Host and the Co-Host will be reassigned as a meeting Co-Host. 

- A Co-Host does not have recording privileges.  
    - While actively recording, if a Host needs to leave the session, then they must assign a Co-Host to be the new Host, prior to exiting the meeting in order for the recording to resume. If a recording is stopped/paused as previously mentioned, then a new recording file is created and saved (multiple recording files would/could then exist for a single session).
